...
use super::coords::{CursorPos, Selection, TAB_WIDTH, document, line};
use super::folding::FoldState;
use super::highlight::{SyntaxLanguage, SyntaxToken, TokenKind};
use super::search::SearchState;
use super::undo::{EditKind, UndoConfig, UndoStack};
use super::wrap::{self, WrapConfig};
use regex::{Captures, RegexBuilder};
use ropey::Rope;

use state::{DocumentState, SessionState};
...
impl Buffer {
	...
	pub fn secondary_carets(&self) -> &[CursorPos] {
		&[]
	}
	...
	pub fn add_caret(&mut self, pos: CursorPos) {
		if !self.session.selection.is_caret() {
			return;
		}
	...
	fn all_carets(&self) -> Vec<CursorPos> {
	...
	fn set_carets(&mut self, primary: CursorPos, secondary: Vec<CursorPos>) {
	...
	fn map_secondary_carets(&mut self, mut map: impl FnMut(&Self, CursorPos) -> CursorPos) {
		...
		let secondary = self
			...
			.collect();
		self.set_carets(primary, secondary);
	}
	...
	pub fn cut(&mut self) -> String {
		...
		if self.has_secondary_selections() {
			...
			if !ranges.is_empty() {
				...
				self.set_carets(primary_start, secondary);
				self.post_edit();
			}
	...
	pub fn paste(&mut self, text: &str) {
		...
		if self.has_secondary_selections() {
			...
			self.set_carets(new_primary, secondary);
			self.post_edit();
			return;
		}
		if self.has_secondary_carets() {
	...
	pub fn indent_lines(&mut self) {
		if self.has_secondary_carets() {
			...
			let mut lines: Vec<usize> = self.all_carets().into_iter().map(|p| p.line).collect();
			lines.sort_unstable();
			lines.dedup();
			...
			self.map_secondary_carets(|_, p| CursorPos::new(p.line, p.col + 1));
			self.post_edit();
			return;
		}
	...
	pub fn insert_newline(&mut self) {
		if self.has_secondary_carets() {
			...
			let primary_col = edits
				...
				.map(|(_, _, _, col)| *col)
				.unwrap_or(0);
			self.set_carets(CursorPos::new(primary.line + 1, primary_col), secondary);
			self.post_edit();
			return;
		}
		self.save_undo(EditKind::Newline);
		...
		if self.has_secondary_carets() {
			if let Some(close) = matching_close(ch) {
				...
				let secondary = edits
					...
					.filter(|&caret| caret != primary)
					.collect();
				self.set_carets(CursorPos::new(primary.line, primary.col + 1), secondary);
				self.post_edit();
				return;
			}
			self.insert_char(ch);
	...
	pub fn duplicate_line(&mut self) {
		if self.has_secondary_carets() {
			...
			let secondary = self
				...
				.collect();
			self.set_carets(shifted(primary), secondary);
			self.post_edit();
			return;
		}
		self.save_undo_boundary();
	...
	pub fn move_left(&mut self, extend: bool) {
		if self.has_secondary_carets() && extend {
			self.session.desired_col = None;
		...
		if self.has_secondary_carets() && !extend {
			self.session.desired_col = None;
			self.map_secondary_carets(|buf, p| buf.move_caret_left(p));
			return;
		}
		self.session.desired_col = None;
	...
	pub fn move_right(&mut self, extend: bool) {
		...
		if self.has_secondary_carets() && !extend {
			self.session.desired_col = None;
			self.map_secondary_carets(|buf, p| buf.move_caret_right(p));
			return;
		}
		self.session.desired_col = None;
		if !extend && !self.session.selection.is_caret() {
	...
	pub fn move_up(&mut self, extend: bool) {
		...
		if self.has_secondary_carets() && !extend {
			let target_col = self
				...
				.desired_col
				.unwrap_or(self.session.selection.head.col);
			self.map_secondary_carets(|buf, p| buf.move_caret_up(p, target_col));
			self.session.desired_col = Some(target_col);
			return;
		}
		let p = self.session.selection.head;
	...
	pub fn move_down(&mut self, extend: bool) {
		...
		if self.has_secondary_carets() && !extend {
			let target_col = self
				...
				.desired_col
				.unwrap_or(self.session.selection.head.col);
			self.map_secondary_carets(|buf, p| buf.move_caret_down(p, target_col));
			self.session.desired_col = Some(target_col);
			return;
		}
		let p = self.session.selection.head;
	...
	pub fn move_home(&mut self, extend: bool) {
		...
		if self.has_secondary_carets() && !extend {
			self.session.desired_col = None;
			self.map_secondary_carets(|buf, p| {
				let first = buf
					.line_text(p.line)
					.chars()
					.position(|c| !c.is_whitespace())
	...
	pub fn move_end(&mut self, extend: bool) {
		...
		if self.has_secondary_carets() && !extend {
			self.session.desired_col = None;
			self.map_secondary_carets(|buf, p| CursorPos::new(p.line, buf.line_len(p.line)));
			return;
		}
		self.session.desired_col = None;
		let p = self.session.selection.head;
	...
	pub fn move_word_left(&mut self, extend: bool) {
		...
		if self.has_secondary_carets() && !extend {
			self.session.desired_col = None;
			self.map_secondary_carets(|buf, p| buf.word_boundary_left(p));
			return;
		}
		self.session.desired_col = None;
	...
	pub fn move_word_right(&mut self, extend: bool) {
		...
		if self.has_secondary_carets() && !extend {
			self.session.desired_col = None;
			self.map_secondary_carets(|buf, p| buf.word_boundary_right(p));
			return;
		}
		self.session.desired_col = None;
		self.set_head(
	...
	pub fn move_to_start(&mut self, extend: bool) {
		if self.has_secondary_carets() && !extend {
			self.session.desired_col = None;
			self.map_secondary_carets(|_buf, _| CursorPos::zero());
			return;
		}
		self.session.desired_col = None;
	...
	pub fn move_to_end(&mut self, extend: bool) {
		if self.has_secondary_carets() && !extend {
			self.session.desired_col = None;
			self.map_secondary_carets(|buf, _| {
