...
impl<'a, Message> SqlEditor<'a, Message> {
    ...
    fn draw_line_gutter(&self, renderer: &mut Renderer, b: Rectangle, gw: f32, li: usize, y: f32, active: usize) {
        ...
        draw_text(renderer, &num, b.x + gw - FOLD_COL_W - GUTTER_PAD - (num.len() as f32 * CHAR_W), y, nc, gw);
        if self.buffer.folds.is_foldable(li) {
            let collapsed = self.buffer.folds.is_collapsed_start(li);
            draw_text(renderer, if collapsed { "▶" } else { "▼" }, b.x + gw - FOLD_COL_W + 2.0, y, th.fold_indicator, FOLD_COL_W);
        }
    ...
    fn draw_line_highlights(&self, renderer: &mut Renderer, b: Rectangle, gw: f32, tx: f32, li: usize, y: f32, active: usize, st: &EditorState) {
        ...
        if li == active && st.is_focused {
            fill(renderer, Rectangle {
                x: b.x + gw, y,
                width: b.width - gw - SCROLL_W - if self.show_minimap { MINIMAP_W } else { 0.0 },
                height: LINE_H,
            }, th.current_line_bg);
        }
        ...
        for li in 0..self.buffer.line_count() {
            ...
            for tok in self.buffer.tokens() {
                ...
                if tw > 0.5 {
                    ...
                    fill(renderer, Rectangle { x: mx + 4.0 + s as f32 * MINIMAP_CHAR_W, y: my, width: tw.min(MINIMAP_W - 8.0), height: MINIMAP_LINE_H }, Color::from_rgba(c.r, c.g, c.b, 0.35));
                }
            }
        }
    }
